...
#--Model 2 Adjusting for LRKK2

library("lme4",lib.loc="/home/ss5505/R_LIB/")

library("argparse")
parser <- ArgumentParser(description="Run model 1 for PD analysis")
...
tryCatch({
	...
	exon_mod[i,2]<-summary( glmer(PD~as.factor(SEX)+AGE_SCALE+as.numeric(as.character(PDmerge[,i]))+(1|FID) +as.factor(LRKK2)+ (1|STUDY) + RIN , data=PDmerge,family = binomial(link=logit)) )$coeff[4,4]	
	},
	error=function(error_message){
	message(error_message)
	return(NA)
