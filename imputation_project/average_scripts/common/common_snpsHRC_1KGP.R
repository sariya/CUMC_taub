...
write.table(as.data.frame(uncommon %>% group_by(panel) %>% summarize_at(vars(info),mean)),"info_uncommon",quote=FALSE,row.names=FALSE, col.names=TRUE,sep="\t")
write.table(as.data.frame(rare %>% group_by(panel) %>% summarize_at(vars(info),mean)),"info_rare",quote=FALSE,row.names=FALSE, col.names=TRUE,sep="\t")
write.table(as.data.frame(ultra_rare %>% group_by(panel) %>% summarize_at(vars(info),mean)),"info_ultra_rare",quote=FALSE,row.names=FALSE, col.names=TRUE,sep="\t")

print("Compared for all commmon SNPs")

info_uncommon<-merged_dfs[which(merged_dfs$exp_freq_a1<=0.05 & merged_dfs$info>=0.40),] # 40 perc 
...
write.table(as.data.frame(info_uncommon %>% group_by(panel) %>% summarize_at(vars(info),mean)),"0.4info_uncommon",quote=FALSE,row.names=FALSE, col.names=TRUE,sep="\t")
...
write.table(mean_all,"boxPlots_all",quote=FALSE,row.names=FALSE, col.names=TRUE,sep="\t")

##################################
binned_snps<-transform(merged_dfs,binned=cut(merged_dfs$exp_freq_a1, include.lowest = TRUE,seq(0, 0.05, by=0.005)))
binned_snps<-binned_snps[complete.cases(binned_snps),]
mean_uncommon<-as.data.frame(binned_snps %>% group_by(binned,panel) %>% summarize_at(vars(info),mean))
write.table(mean_uncommon,"boxPlots_uncommon",quote=FALSE,row.names=FALSE, col.names=TRUE,sep="\t")
##################################
binned_snps<-transform(merged_dfs,binned=cut(merged_dfs$exp_freq_a1,include.lowest = TRUE, seq(0, 0.01, by=0.001)))
binned_snps<-binned_snps[complete.cases(binned_snps),]
mean_ultra<-as.data.frame(binned_snps %>% group_by(binned,panel) %>% summarize_at(vars(info),mean))
write.table(mean_ultra,"boxPlots_ultra",quote=FALSE,row.names=FALSE, col.names=TRUE,sep="\t")
...
binned_snps<-binned_snps[complete.cases(binned_snps),]
mean_all<-as.data.frame(binned_snps %>% group_by(binned,panel) %>% summarize_at(vars(info),mean))
write.table(mean_all,"0.40boxPlots_all",quote=FALSE,row.names=FALSE, col.names=TRUE,sep="\t")
print("done with All MAF")
##################################
binned_snps<-transform(merged_dfs,binned=cut(merged_dfs$exp_freq_a1, include.lowest = TRUE,seq(0, 0.05, by=0.005)))
binned_snps<-binned_snps[complete.cases(binned_snps),]
mean_uncommon<-as.data.frame(binned_snps %>% group_by(binned,panel) %>% summarize_at(vars(info),mean))
write.table(mean_uncommon,"0.40boxPlots_uncommon",quote=FALSE,row.names=FALSE, col.names=TRUE,sep="\t")
print("done with 5% MAF")
##################################
binned_snps<-transform(merged_dfs,binned=cut(merged_dfs$exp_freq_a1,include.lowest = TRUE, seq(0, 0.01, by=0.001)))
binned_snps<-binned_snps[complete.cases(binned_snps),]
mean_ultra<-as.data.frame(binned_snps %>% group_by(binned,panel) %>% summarize_at(vars(info),mean))
write.table(mean_ultra,"0.40boxPlots_ultra",quote=FALSE,row.names=FALSE, col.names=TRUE,sep="\t")
print("Done with ultra rare")
