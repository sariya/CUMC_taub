...
#    bash /home/ss5505/scripts_cumc/gwas_/phasing_scripts/launch_job_chrm_ph.sh -b $b_file -m $m_file -f $hap_file -l $legend_file -s $sample_file -p $chrom_pre$chr_num
    ...
    qsub -wd "$input_dir/$chrom_pre$chr_num/" -l h_vmem=15G -N "ph_chr""$chr_num" -v bash /mnt/mfs/hgrcgrid/homes/ss5505/scripts_cumc/gwas_/phasing_scripts/1KGP/launch_job_chrm_ph.sh -b $b_file -m $m_file -f $hap_file -l $legend_file -s $sample_file -p $chrom_pre$chr_num
