...
parser$add_argument('-x',"--pre",help="input dosage per SNPs",required=TRUE)  # output file name
...
#snp_dosage.file<-"test_dosage_convert_CHR22" #this is comma separated file  #ancestry_component.file<-"test_NAT_CHR22" #this is comma separated file
...
for (i in 1:nrow(df.ancestry)){
    ...
    snp_name<-unlist(snp_name) #this is pain when playing data from data.table !!! :-/
    
    if(length(which(df.ancestry$V1==snp_name))!=0){
        ...
        tryCatch({
            ##V1 is from dosage V2 is from ancestry
            temp.df_pvalue<-summary(glm(AD  ~ as.numeric(as.character(V1)) + as.factor(as.numeric(as.character(V2))) + as.numeric(as.character(MDS_1))+as.numeric(as.character(MDS_2))  + as.numeric(as.character(age)) + as.factor(sex) +as.numeric(as.character(NAT)), 
        ...
        print(paste("missing SNP ",snp_name))
    }
...
write.table(store_pvaluePerSNP, file = output_pre, append = FALSE, quote = FALSE, sep = "\t", eol = "\n",
